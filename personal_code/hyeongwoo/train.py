...
def train(data_dir, model_dir, args):
    ...
    save_dir = increment_path(os.path.join(model_dir, f'{args.model}_{args.epochs}_{args.batch_size}'))
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            if (idx + 1) % args.log_interval == 0:
                ...
                loss_value = 0
                matches = 0
        wandb.log({"training_loss" : train_loss, "training_acc" : train_acc}, step = epoch)
        scheduler.step()
        # val loop
        with torch.no_grad():
            ...
            print(
                f"[Val] acc : {val_acc:4.2%}, loss : {val_loss:4.2}  val_f1 : {val_f1:4.2%} || "
                f"best acc : {best_val_acc:4.2%}, best loss: {best_val_loss:4.2}"
            )
            wandb.log({"val_loss" : val_loss, "val_acc" : val_acc, "val_f1" : val_f1}, step = epoch)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description = "baseline 모델 train")
    ...
    wandb.init(project = "final_model")
    wandb.run.name = f'model : {args.model} size : {args.batch_size}'
